...
def my_code():
    ...
    alpha_rod1 = 1e-3  # thermal diffusivity for rod 1
    alpha_rod2 = 1e-4  # thermal diffusivity for rod 2
    dt = 0.1  # time step
...
def plot_contour(T_total, x_values_total, duration, Nt):
    ...
    plt.ylabel('Time (s)')
    plt.title('Heat Diffusion in Several Connected Rods of Different Materials')
    plt.savefig('Heat_diffusion_in_metal_rods/Plots/HeatDiff_two_connected_rods_mBC_contour.png', dpi=300)
